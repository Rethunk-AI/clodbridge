 ...
 */
export function registerSkillsResources(server: McpServer, reader: CursorReader): void {
  ...
  server.resource(
    ...
    async (uri) => {
      try {
        ...
        throw new Error(
          `Failed to generate skills index: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    },
  );
  ...
  server.resource(
    ...
    async (uri, params: Record<string, unknown>) => {
      try {
        ...
        throw new Error(`Failed to get skill: ${err instanceof Error ? err.message : String(err)}`);
      }
    },
  );
}
