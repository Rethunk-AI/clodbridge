 ...
 */
export function registerRulesResources(server: McpServer, reader: CursorReader): void {
  ...
  server.resource(
    ...
    async (uri) => {
      try {
        ...
        throw new Error(
          `Failed to generate rules index: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    },
  );
  ...
  server.resource(
    ...
    async (uri, params: Record<string, unknown>) => {
      try {
        ...
        throw new Error(`Failed to get rule: ${err instanceof Error ? err.message : String(err)}`);
      }
    },
  );
}
