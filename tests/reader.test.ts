...
Content`,
      ...
      // Add a new rule and trigger multiple concurrent reloads
      await writeFile(
        path.join(rulesDir, "second.mdc"),
        `---
...
Content`,
      ...
      // Fire multiple reloads concurrently
      const reloadPromises = [reader.reload(), reader.reload(), reader.reload()];

      await Promise.all(reloadPromises);

      // Should have both rules (not duplicates or errors)
      expect(reader.store.rules.size).toBe(2);
      expect(reader.store.rules.has("initial")).toBe(true);
      expect(reader.store.rules.has("second")).toBe(true);
    } finally {
      await rm(testDir, { recursive: true, force: true });
    }
  });
