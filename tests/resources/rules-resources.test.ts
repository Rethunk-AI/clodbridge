...
class MockMcpServer {
  ...
  resource(
    ...
    // Handle overloaded signature: resource(name, uri, handler) or resource(name, uri, opts, handler)
  ...
  async callResource(uriString: string, params?: Record<string, unknown>) {
    // Find matching resource handler (support URI templates like cursor://rules/{name})
    ...
    if (this.resources.has(uriString)) {
      ...
      // Check for template match (e.g., cursor://rules/{name})
...
describe('Rules MCP Resources', () => {
  ...
  describe('cursor://rules (index resource)', () => {
    ...
    it('properly categorizes rule modes', async () => {
      ...
      expect(alwaysRules.some((r: any) => r.name === 'always-rule')).toBe(
        true
      );
    });
  });
  ...
  describe('cursor://rules/{name} (per-rule resource)', () => {
    ...
    it('throws on nonexistent rule', async () => {
      await expect(
        server.callResource('cursor://rules/nonexistent-rule')
      ).rejects.toThrow();
    });
    ...
    it('handles rule name case-sensitively', async () => {
      ...
      await expect(
        server.callResource('cursor://rules/ALWAYS-RULE')
      ).rejects.toThrow();
    });
